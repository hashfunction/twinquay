...
class DupeGuru(QObject):
    ...
    def _setupActions(self):
        ...
        ACTIONS = [
            ("actionImportLegacySettings", "", "", "Import dupeGuru scan settings…", self.import_legacy_settings),
            ("actionQuarantineReceipts", "Ctrl+Shift+Q", "", "Quarantine Receipts…", self.show_quarantine_receipts),
    ...
    def import_legacy_settings(self):
        filename, _ = QFileDialog.getOpenFileName(
            self.main_window or self.directories_dialog,
            "Choose dupeGuru settings.ini to import scan preferences",
            "",
            "Settings (*.ini)",
        )
        if filename:
            try:
                count = self.prefs.import_scan_preferences(filename)
                self._update_options()
                self.model.view.show_message(f"Imported {count} scan preferences. The old settings file was kept.")
            except (OSError, ValueError) as exc:
                self.model.view.show_message(str(exc))

    def show_cleanup_plan(self, plan):
        CleanupPlanDialog(self.main_window or self.directories_dialog, self, plan).exec()

    def show_quarantine_receipts(self):
        QuarantineDialog(self.main_window or self.directories_dialog, self).exec()
