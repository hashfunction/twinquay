...
class ProgressWindow(GUIObject, ThreadedJobPerformer):
        ...
        if not self._job_running or last_progress is None:
            ...
            if self.last_error is not None:
                ...
                if self._error_func is not None:
                    ...
                    raise err
            if (not self.job_cancelled or self.complete_on_cancel) and should_continue:
    ...
    def run(self, jobid: str, title: str, target: Callable, args: Tuple = (), complete_on_cancel=False):
        ...
        j = self.create_job()  # Check for a running job before changing its completion state.
        self.jobid = jobid
