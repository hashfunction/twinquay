...
import os
from importlib.machinery import PathFinder, SOURCE_SUFFIXES, SourceFileLoader
import sys
import glob
import token
import tokenize
...
def _get_modpkg_path(dotted_name, pathlist=None):
    ...
    spec = PathFinder.find_spec(parts[0], pathlist)
    if spec is None:
        return None
    if len(parts) > 1:
        if spec.submodule_search_locations is None:
            return None
        return _get_modpkg_path(parts[1], spec.submodule_search_locations)
    if spec.submodule_search_locations:
        return list(spec.submodule_search_locations)[0]
    return spec.origin if isinstance(spec.loader, SourceFileLoader) else None
...
def getFilesForName(name):
    ...
    if os.path.isdir(name):
        ...
        file_list = []
        for dirname, dirs, names in os.walk(name):
            if "CVS" in dirs:
                dirs.remove("CVS")
            _visit_pyfiles(file_list, dirname, names)
        return file_list
