...
from core.cleanup_plan import CleanupCandidate, CleanupPlan, EvidenceKind, path_key
...
JOBID2TITLE = {
    JobType.CLEANUP: tr("Verifying and quarantining"),
    JobType.RESTORE: tr("Restoring quarantine"),
...
class DupeGuru(Broadcaster):
    ...
    def _start_job(self, jobid, func, args=()):
        ...
        try:
            self.progress_window.run(
                jobid, title, func, args=args, complete_on_cancel=jobid in {JobType.CLEANUP, JobType.RESTORE}
            )
            return True
    ...
    def _job_completed(self, jobid):
        if jobid in {JobType.CLEANUP, JobType.RESTORE}:
            receipt = self.last_cleanup_receipt
            if receipt is not None:
                if jobid == JobType.CLEANUP:
                    moved = {path_key(item.original_path) for item in receipt.items if item.status == "quarantined"}
                    self.results.remove_duplicates([d for d in self.results.dupes if path_key(d.path) in moved])
                ...
                self.view.show_message(
                    f"{summary}\nReceipt: {receipt.receipt_path}\nUse Quarantine Receipts to inspect or restore. Rescan after restore."
                )
            return
    ...
    def _job_error(self, jobid, err):
        if jobid in {JobType.CLEANUP, JobType.RESTORE}:
            self.view.show_message(
                f"Operation stopped: {err}\nKeep the quarantine folder. Open its receipt.json to inspect recovery state."
            )
            return False
        if jobid == JobType.LOAD:
    ...
    def build_cleanup_plan(self, marked=None):
        if marked is None:
            marked = [d for d in self.results.dupes if self.results.is_marked(d)]
        evidence = (
            EvidenceKind.EXACT_CONTENT if self.results_scan_type == ScanType.CONTENTS else EvidenceKind.SIMILARITY
        )
        candidates = []
        for dupe in marked:
            group = self.results.get_group_of_duplicate(dupe)
            if group is not None and group.ref is not dupe:
                candidates.append(CleanupCandidate.capture(dupe.path, group.ref.path, evidence))
        return CleanupPlan.create(candidates)

    def preview_cleanup_plan(self):
        plan = self.build_cleanup_plan()
        if not plan.candidates:
            self.view.show_message(MSG_NO_MARKED_DUPES)
            return False
        self.view.show_cleanup_plan(plan)
        return True

    def delete_marked(self):
        """The former delete entrypoint only opens the reviewable quarantine plan."""
        return self.preview_cleanup_plan()

    def _cleanup_progress(self, done, total, path):
        # Updating job state directly avoids JobCancelled between write-ahead checkpoints.
        self.progress_window.last_progress = int(100 * done / max(total, 1))
        self.progress_window.last_desc = str(path)

    def execute_cleanup_plan(self, plan, quarantine_root):
        if not plan.candidates or not any(c.evidence == EvidenceKind.EXACT_CONTENT for c in plan.candidates):
            self.view.show_message("No exact-byte candidates. Run a Contents scan and review its matches.")
            return False
        if not quarantine_root:
            self.view.show_message("Choose a quarantine folder before executing the plan.")
            return False

        def do(j):
            # Keep the selected plan in independent app data before moving any input.
            plan_dir = Path(self.appdata) / "cleanup-plans"
            plan_dir.mkdir(parents=True, exist_ok=True)
            with (plan_dir / f"{plan.plan_id}.json").open("x", encoding="utf-8") as stream:
                json.dump(plan.to_dict(), stream, ensure_ascii=True, indent=2)
                stream.flush()
                os.fsync(stream.fileno())
            self.last_cleanup_receipt = execute_plan(
                plan, Path(quarantine_root), lambda: self.progress_window.job_cancelled, self._cleanup_progress
            )
            # A small durable index lets the UI reopen user-selected quarantine folders.
            with (plan_dir / f"{plan.plan_id}.receipt.txt").open("w", encoding="utf-8") as stream:
                stream.write(str(self.last_cleanup_receipt.receipt_path))

        return self._start_job(JobType.CLEANUP, do)

    def restore_quarantine(self, receipt_path, item_ids):
        if not item_ids:
            return False

        def do(j):
            self.last_cleanup_receipt = restore_receipt(
                Path(receipt_path), item_ids, lambda: self.progress_window.job_cancelled, self._cleanup_progress
            )

        return self._start_job(JobType.RESTORE, do)
    ...
    def load_from(self, filename):
        ...
        def do(j):
            self.results_scan_type = None  # Saved upstream results carry no trusted scan evidence.
