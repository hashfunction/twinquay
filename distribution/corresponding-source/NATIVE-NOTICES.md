...
## Qt PDF attribution evidence

The acquired QtWebEngine **6.11.2** source records Chromium base **140.0.7339.264**, security patches through **151.0.7922.71**, and PDFium revision **1afaa1a380fcd06cec420f3e5b6ec1d2ccb920dc**. All nine projects listed by the matching Qt PDF licensing page have source notice texts in this supplemental tree: PDFium, Chromium, Abseil, fast_float, FreeType, ICU, libjpeg-turbo, libpng and zlib. Nested PDFium source-license references are included conservatively as well. The declarations marked `Shipped: no` in `pdf-source-versions.json` retain that status. [Qt PDF 6.11.2 licensing](https://doc.qt.io/qt-6/qtpdf-licensing.html).

The exact source `src/pdf/CMakeLists.txt` defines `generate_pdf_attributions`, producing `pdf_attributions.qdoc` from GN target `:QtPdf` and the configured build directory. `src/pdf/configure.cmake` defaults V8 auto-detection off; XFA depends on V8, and its BMP/GIF/PNG/TIFF options depend on XFA. Those source defaults do **not** establish the original wheel's flags. The module also marks its third-party SBOM incomplete. The subsequently acquired official PDF 6.11.2 build archive exactly matches both packaged PDF DLLs and supplies their original disabled-feature header, full configure command and SBOM. Its matching documentation build supplies all nine generated attribution pages. See `ORIGINAL-BUILD-EVIDENCE.md` for the measured artifacts and 1,751-file source comparison. These original records resolve the prior configuration question while retaining the binary SBOM's stated third-party coverage limits. `REBUILD.md` describes retaining new attributions if a user changes the library configuration.

## Microsoft files redistributed

The current baseline's version/signature observations identify six files at **14.44.35211.0**: the Qt wheel's MSVCP140, MSVCP140_1, MSVCP140_2, VCRUNTIME140 and VCRUNTIME140_1 plus pywin32's MFC140U. Python supplies two VCRUNTIME files at **14.42.34438.0**. All eight signatures were observed `Valid`, and all eight bytes match the exact comparison inputs. The exact Microsoft Runtime, Visual Studio 2022 Community and Build Tools agreements have now been retained and their distinct roles reviewed in `ORIGINAL-BUILD-EVIDENCE.md`.

The exact Python source's `PC/crtlicense.txt` is now included and preserves the Windows binary redistribution conditions, expressly scoped to Microsoft Distributable Code. Microsoft's Visual Studio redistribution list covers VC redist files subject to its applicable license terms, and distinguishes prohibited debug runtime files. Community section 4 includes downstream application-distributor permission. The Runtime agreement supplies end-user use terms and does not independently grant standalone redistribution. The supplemental `release-notices` payload retains the original terms and a scoped distributor notice; Microsoft conditions do not relicense or restrict the GPL application or LGPL libraries. [Microsoft redistribution list](https://learn.microsoft.com/en-us/visualstudio/releases/2022/redistribution).

## Release status

Local preparation and staging checks passed. Run 34677345789 passed both installed identity modes with the source notice checkout fix and Windows system-library policy. The later 15-file original-build/runtime notice supplement requires its own current-source package run. Corresponding-source publication and final unsigned Store export remain unclaimed. `PUBLICATION.md` and `source-release-assets.json` prepare the 71 exact source assets and final application archive procedure. The release must preserve the LGPL source/recombine route and bind final package evidence to verified public source. Historical qualification receipts remain unchanged.
