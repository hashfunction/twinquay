...
Check ([Runtime.InteropServices.Marshal]::SizeOf([DupliSiftMarketing.Mode]::new()) -eq 220 -and
    [Runtime.InteropServices.Marshal]::OffsetOf([DupliSiftMarketing.Mode],'dmPelsWidth').ToInt32() -eq 172 -and
    [Runtime.InteropServices.Marshal]::SizeOf([DupliSiftMarketing.DisplayDevice]::new()) -eq 840) 'Native Unicode display structure layout differs.'
foreach($flag in @(1,0x100,0x200,0x10000000)){
    $failed=$false;try{$null=[DupliSiftMarketing.DisplayModes]::Change('not-a-device',[DupliSiftMarketing.Mode]::new(),$flag)}catch{$failed=$_.Exception.Message -match 'Only native test and dynamic change'}
