...
TwinQuay is Trieflow LLC's Windows duplicate-discovery application, based on
...
[Product](https://twinquay.trieflow.com) ·
[Privacy](https://twinquay.trieflow.com/privacy) ·
[Support](https://twinquay.trieflow.com/support)
...
See [user help](help/twinquay/index.html) and [recovery details](docs/cleanup-recovery.md).

Windows application data is `%LOCALAPPDATA%\TwinQuay`; quarantine remains in the
folder the user chooses. Optional portable mode requires `TwinQuay.ini` beside
the executable and uses `data/TwinQuay`. Old preferences are kept separate. The
...
history are retained. TwinQuay modifications and active icons are copyright
